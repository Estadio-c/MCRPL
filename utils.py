...
def ndcg_k(predicted, actual, topk):
    res = 0
    for user_id in range(len(actual)):
        k = min(topk, len([actual[user_id]]))
        idcg = idcg_k(k)
        # dcg_k = sum([int(predicted[user_id][j] in set([actual[user_id]])) / math.log(j + 2, 2) for j in range(topk)])
        dcg_k = sum([int(predicted[user_id][j] in (actual[user_id]-1)) / math.log(j + 2, 2) for j in range(topk)])
        res += dcg_k / idcg
...
def idcg_k(k):
    ...
    if not res:
        ...
        return res
def get_eval(predlist, truelist, topk):
    recall = 0
    mrr = 0
    templist = predlist[:,:topk]
    i = 0
    while i < len(truelist):
        ...
        if len(pos) > 0:
            ...
            mrr += 0
        i += 1
    return recall/predlist.shape[0], mrr/predlist.shape[0]

def get_full_sort_score(epoch, logits, answers):
    pred_list = torch.flip(logits.argsort(1),dims=[1])
    recall, ndcg, mrr = [], [], []
    for k in [3, 5, 10]:
        recall.append(get_eval(pred_list, answers, k)[0])
        mrr.append(get_eval(pred_list, answers,  k)[1])
        ndcg.append(ndcg_k(pred_list, answers,  k))
    post_fix = {
        "Epoch": epoch,
        "Recall@3": "{:.4f}".format(recall[0]),
        "NDCG@3": "{:.4f}".format(ndcg[0]),
        "Mrr@3": "{:.4f}".format(mrr[0]),
        "Recall@5": "{:.4f}".format(recall[1]),
        "NDCG@5": "{:.4f}".format(ndcg[1]),
        "Mrr@5": "{:.4f}".format(mrr[1]),
        "Recall@10": "{:.4f}".format(recall[2]),
        "NDCG@10": "{:.4f}".format(ndcg[2]),
        "Mrr@10": "{:.4f}".format(mrr[2])
    }
    print(post_fix)